...
function get_path_elements() {
  if (empty($_SERVER['PATH_INFO'])) { return null; }
...
function create_controller($controller_name) {
  ...
  if (!file_exists($controller_filename)) { throw new Exception("$controller_filename controller file does not exist"); }
  include $controller_filename;
  if (!class_exists($controller_classname)) { throw new Exception("$controller_classname class does not exist in $controller_filename"); }
